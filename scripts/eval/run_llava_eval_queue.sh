...
#   echo "All task will be evaluated"
    ...
    # MODEL_PATH="checkpoints/llava-next-llama3-8b-finetune-full-llama3prompt"
    # TRAIN_JOB_ID=29115993
    # MODEL_NAME="llama3"
    # sbatch --job-name=llava_eval_flickr_verbose_${MODEL_NAME} scripts/eval/llava_eval.sbatch $DATA_ROOT $MODEL_PATH $ENCODER $GPU true false false false false false false false english $ATTACK $EPSILON $STEPS
    # sbatch --job-name=llava_eval_coco_verbose_${MODEL_NAME} scripts/eval/llava_eval.sbatch $DATA_ROOT $MODEL_PATH $ENCODER $GPU false true false false false false false false english $ATTACK $EPSILON $STEPS
    # sbatch --job-name=llava_eval_vqav2_verbose_${MODEL_NAME} scripts/eval/llava_eval.sbatch $DATA_ROOT $MODEL_PATH $ENCODER $GPU false false true false false false false false english $ATTACK $EPSILON $STEPS
    # sbatch --job-name=llava_eval_textvqa_verbose_${MODEL_NAME} scripts/eval/llava_eval.sbatch $DATA_ROOT $MODEL_PATH $ENCODER $GPU false false false true false false false false english $ATTACK $EPSILON $STEPS
    # sbatch --job-name=llava_eval_vizwiz_verbose_${MODEL_NAME} scripts/eval/llava_eval.sbatch $DATA_ROOT $MODEL_PATH $ENCODER $GPU false false false false true false false false english $ATTACK $EPSILON $STEPS
    # sbatch --job-name=llava_eval_okvqa_verbose_${MODEL_NAME} scripts/eval/llava_eval.sbatch $DATA_ROOT $MODEL_PATH $ENCODER $GPU false false false false false true false false english $ATTACK $EPSILON $STEPS
    # sbatch --job-name=llava_eval_mt_trust_verbose_${MODEL_NAME} scripts/eval/llava_eval.sbatch $DATA_ROOT $MODEL_PATH $ENCODER $GPU false false false false false false true false english $ATTACK $EPSILON $STEPS
    # sbatch --job-name=llava_eval_eval_multilingual_llavabench_verbose_${MODEL_NAME} scripts/eval/llava_eval.sbatch $DATA_ROOT $MODEL_PATH $ENCODER $GPU false false false false false false false true english $ATTACK $EPSILON $STEPS

    MODEL_PATH="checkpoints/llama3-llava-next-8b"
    TRAIN_JOB_ID=29115993
    MODEL_NAME="llama3"
    sbatch --job-name=llava_eval_flickr_verbose_${MODEL_NAME} scripts/eval/llava_eval.sbatch $DATA_ROOT $MODEL_PATH $ENCODER $GPU true false false false false false false false english $ATTACK $EPSILON $STEPS
    sbatch --job-name=llava_eval_coco_verbose_${MODEL_NAME} scripts/eval/llava_eval.sbatch $DATA_ROOT $MODEL_PATH $ENCODER $GPU false true false false false false false false english $ATTACK $EPSILON $STEPS
    ...
    # MODEL_PATH="checkpoints/llava-next-apertus-8b-finetune-full-apertusprompt"
    # TRAIN_JOB_ID=29119296
    # MODEL_NAME="apertus"
    # sbatch --job-name=llava_eval_flickr_verbose_${MODEL_NAME} scripts/eval/llava_eval.sbatch $DATA_ROOT $MODEL_PATH $ENCODER $GPU true false false false false false false false english $ATTACK $EPSILON $STEPS
    # sbatch --job-name=llava_eval_coco_verbose_${MODEL_NAME} scripts/eval/llava_eval.sbatch $DATA_ROOT $MODEL_PATH $ENCODER $GPU false true false false false false false false english $ATTACK $EPSILON $STEPS
    # sbatch --job-name=llava_eval_vqav2_verbose_${MODEL_NAME} scripts/eval/llava_eval.sbatch $DATA_ROOT $MODEL_PATH $ENCODER $GPU false false true false false false false false english $ATTACK $EPSILON $STEPS
    ...
    # MODEL_PATH="checkpoints/llava-next-qwen25-7b-finetune-full-qwen25prompt"
    # TRAIN_JOB_ID=29116323
    # MODEL_NAME="qwen25"
    # sbatch --job-name=llava_eval_flickr_verbose_${MODEL_NAME} scripts/eval/llava_eval.sbatch $DATA_ROOT $MODEL_PATH $ENCODER $GPU true false false false false false false false english $ATTACK $EPSILON $STEPS
    # sbatch --job-name=llava_eval_coco_verbose_${MODEL_NAME} scripts/eval/llava_eval.sbatch $DATA_ROOT $MODEL_PATH $ENCODER $GPU false true false false false false false false english $ATTACK $EPSILON $STEPS
    # sbatch --job-name=llava_eval_vqav2_verbose_${MODEL_NAME} scripts/eval/llava_eval.sbatch $DATA_ROOT $MODEL_PATH $ENCODER $GPU false false true false false false false false english $ATTACK $EPSILON $STEPS
